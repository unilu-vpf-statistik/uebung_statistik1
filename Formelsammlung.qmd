...
$$
\text{gerichtete Hypothese:} \quad |T_{krit}| = F^{-1}(p = 1-\alpha/2) 
$$

$$
\text{ungerichtete Hypothese:} \quad |T_{krit}| = F^{-1}(p = 1-\alpha)
$$

mit der Quantilfunktion $F^{-1}$ der entsprechenden Verteilung des jeweiligen Teststatistik.
...
$$
\text{gerichtete Hypothese:} \quad p = 2\cdot (1 - F(|T_{emp})|) 
$$

$$
\text{ungerichtete Hypothese:} \quad p = 1 - F(|T_{emp}|)
$$
...
$$
\sigma_{\bar{x}_1 - \bar{x}_2}^2 = \sqrt{\frac{\hat{\sigma}_{inn}^2}{N_1} + \frac{\hat{\sigma}_{inn}^2}{N_2}}
$$
